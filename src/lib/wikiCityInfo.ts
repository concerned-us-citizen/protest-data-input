 ...
 */
export async function fetchWikiCityInfo(city: string, state: string): Promise<WikiCityInfo | null> {
	const fullStateName = (state.length === 2 ? getStateInfo(state)?.fullName : state) ?? state;
	...
	} else if (nationalSites.some((siteName) => loweredCity.includes(`national ${siteName}`))) {
		searchQueries = [normalizedCity];
	} else {
		searchQueries = [`${city}, ${fullStateName}`];
		if (normalizedCity.includes('island') || normalizedCity.includes('township')) {
