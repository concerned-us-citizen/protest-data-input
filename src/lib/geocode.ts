...
const USER_AGENT = 'ProtestMap/1.0 (hypo.30-buggy@icloud.com)';
const DELAY_MS = 1000;
...
async function attemptGeocodeWithProps(addr: Address, propsToAttempt: (keyof Address)[]) {
...
export async function geocodeFromService({ address, zip, city, state }: Address): Promise<Geocode> {
	log(`Geocoding ${address ? address : ''} ${city} ${state} ${zip}...`);
